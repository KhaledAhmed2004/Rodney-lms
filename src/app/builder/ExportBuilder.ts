...
class ExportBuilder<T extends Record<string, any>> {
  ...
  async toBuffer(): Promise<Buffer> {
    return traceOperation('ExportBuilder', 'toBuffer', async () => {
      ...
      try {
        ...
        switch (this.config.format) {
          case 'excel':
            recordSpanEvent('generating_excel');
            buffer = await this.generateExcel();
            break;
          case 'csv':
            recordSpanEvent('generating_csv');
            buffer = await this.generateCSV();
            break;
          case 'json':
            recordSpanEvent('generating_json');
          ...
          default:
            throw new BuilderError(
              `Unsupported export format: ${this.config.format}`,
              'ExportBuilder',
              'toBuffer'
            );
        }

        addSpanAttributes({ 'export.size': buffer.length });
        return buffer;
      } catch (error) {
        if (error instanceof BuilderError) {
          throw error;
        }
        const builderError = BuilderError.from(error, 'ExportBuilder', 'toBuffer', {
