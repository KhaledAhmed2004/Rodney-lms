...
export class NotificationBuilder {
  ...
  constructor(options?: INotificationBuilderOptions) {
    const config = getBuilderConfig().notification;
    this.options = {
  ...
  async sendNow(): Promise<INotificationResult> {
    return traceOperation('NotificationBuilder', 'sendNow', async (span) => {
      // Resolve users
      ...
      addSpanAttributes({
        ...
        'notification.template': this.template?.name || 'custom',
      });

      if (users.length === 0) {
        recordSpanEvent('no_recipients');
        return {
          success: true,
      ...
      if (this.channels.has('push')) {
        try {
          recordSpanEvent('channel_start', { channel: 'push' });
          ...
          recordSpanEvent('channel_complete', { channel: 'push', sent: pushResult.sent });
        } catch (error) {
          const builderError = BuilderError.from(error, 'NotificationBuilder', 'sendPush', {
            recipients: users.length,
          });
          builderError.log();
          recordSpanEvent('channel_error', { channel: 'push', error: builderError.message });
      ...
      if (this.channels.has('socket')) {
        try {
          recordSpanEvent('channel_start', { channel: 'socket' });
          ...
          recordSpanEvent('channel_complete', { channel: 'socket', sent: socketResult.sent });
        } catch (error) {
          const builderError = BuilderError.from(error, 'NotificationBuilder', 'sendSocket', {
            recipients: users.length,
          });
          builderError.log();
          recordSpanEvent('channel_error', { channel: 'socket', error: builderError.message });
      ...
      if (this.channels.has('email')) {
        try {
          recordSpanEvent('channel_start', { channel: 'email' });
          ...
          recordSpanEvent('channel_complete', { channel: 'email', sent: emailResult.sent });
        } catch (error) {
          const builderError = BuilderError.from(error, 'NotificationBuilder', 'sendEmail', {
            recipients: users.length,
            template: resolvedContent.email.template,
          });
          builderError.log();
          recordSpanEvent('channel_error', { channel: 'email', error: builderError.message });
      ...
      if (this.channels.has('database')) {
        try {
          recordSpanEvent('channel_start', { channel: 'database' });
          ...
          recordSpanEvent('channel_complete', { channel: 'database', sent: dbResult.sent });
        } catch (error) {
          const builderError = BuilderError.from(error, 'NotificationBuilder', 'saveToDatabase', {
            recipients: users.length,
          });
          builderError.log();
          recordSpanEvent('channel_error', { channel: 'database', error: builderError.message });
      ...
      addSpanAttributes({
        ...
        'notification.sent.total':
          result.sent.push + result.sent.socket + result.sent.email + result.sent.database,
