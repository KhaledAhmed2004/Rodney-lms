...
class PDFBuilder {
  ...
  async toBuffer(): Promise<Buffer> {
    return traceOperation('PDFBuilder', 'toBuffer', async (span) => {
      const config = getBuilderConfig().pdf;
      ...
      try {
        ...
        recordSpanEvent('processing_qr_barcodes');
        await this.processQRCodesAndBarcodes();

        recordSpanEvent('building_html');
        const html = this.buildHTML();
        await page.setContent(html, { waitUntil: 'networkidle0', timeout: config.puppeteerTimeout });

        recordSpanEvent('generating_pdf');
        ...
        addSpanAttributes({ 'pdf.size': buffer.length });

        return buffer;
      } catch (error) {
        const builderError = BuilderError.from(error, 'PDFBuilder', 'toBuffer', {
  ...
  async toFile(path: string): Promise<void> {
    return traceOperation('PDFBuilder', 'toFile', async () => {
      try {
        const fs = await import('fs/promises');
        const buffer = await this.toBuffer();
        await fs.writeFile(path, buffer);
        addSpanAttributes({ 'pdf.filePath': path });
      } catch (error) {
        const builderError = BuilderError.from(error, 'PDFBuilder', 'toFile', { path });
        builderError.log();
        throw builderError;
      }
    });
  ...
  async toBase64(): Promise<string> {
    return traceOperation('PDFBuilder', 'toBase64', async () => {
      try {
        const buffer = await this.toBuffer();
        return buffer.toString('base64');
      } catch (error) {
        const builderError = BuilderError.from(error, 'PDFBuilder', 'toBase64');
        builderError.log();
        throw builderError;
      }
    });
...
<body>
  ...
  private async generateQRCode(config: QRCodeConfig): Promise<string> {
    ...
    try {
      ...
      recordSpanEvent('qrcode_generated', { dataLength: data.length });
      return dataUrl;
    } catch (error) {
      const builderError = BuilderError.from(error, 'PDFBuilder', 'generateQRCode', {
  ...
  private generateBarcode(config: BarcodeConfig): string {
    ...
    try {
      ...
      recordSpanEvent('barcode_generated', { format, dataLength: data.length });
      return canvas.toDataURL('image/png');
    } catch (error) {
      const builderError = BuilderError.from(error, 'PDFBuilder', 'generateBarcode', {
        format,
        data,
      });
      builderError.log('warn');
