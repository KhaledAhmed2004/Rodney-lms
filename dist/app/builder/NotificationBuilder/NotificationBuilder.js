...
class NotificationBuilder {
    ...
    constructor(options) {
        ...
        const config = (0, builderConfig_1.getBuilderConfig)().notification;
        this.options = Object.assign({ defaultChannels: config.defaultChannels, throwOnError: config.throwOnError }, options);
    ...
    sendNow() {
        return __awaiter(this, void 0, void 0, function* () {
            return (0, builderTracing_1.traceOperation)('NotificationBuilder', 'sendNow', (span) => __awaiter(this, void 0, void 0, function* () {
                var _a;
                // Resolve users
                const users = yield this.resolveUsers();
                // Add span attributes
                (0, builderTracing_1.addSpanAttributes)({
                    'notification.recipients': users.length,
                    'notification.channels': Array.from(this.channels).join(','),
                    'notification.template': ((_a = this.template) === null || _a === void 0 ? void 0 : _a.name) || 'custom',
                });
                if (users.length === 0) {
                    (0, builderTracing_1.recordSpanEvent)('no_recipients');
                    return {
                        success: true,
                ...
                if (this.channels.has('push')) {
                    try {
                        (0, builderTracing_1.recordSpanEvent)('channel_start', { channel: 'push' });
                        ...
                        (0, builderTracing_1.recordSpanEvent)('channel_complete', { channel: 'push', sent: pushResult.sent });
                    }
                    catch (error) {
                        const builderError = BuilderError_1.BuilderError.from(error, 'NotificationBuilder', 'sendPush', {
                            recipients: users.length,
                        });
                        builderError.log();
                        (0, builderTracing_1.recordSpanEvent)('channel_error', { channel: 'push', error: builderError.message });
                ...
                if (this.channels.has('socket')) {
                    try {
                        (0, builderTracing_1.recordSpanEvent)('channel_start', { channel: 'socket' });
                        ...
                        (0, builderTracing_1.recordSpanEvent)('channel_complete', { channel: 'socket', sent: socketResult.sent });
                    }
                    catch (error) {
                        const builderError = BuilderError_1.BuilderError.from(error, 'NotificationBuilder', 'sendSocket', {
                            recipients: users.length,
                        });
                        builderError.log();
                        (0, builderTracing_1.recordSpanEvent)('channel_error', { channel: 'socket', error: builderError.message });
                        if (this.options.throwOnError) {
                            throw builderError;
                        }
                ...
                if (this.channels.has('email')) {
                    try {
                        (0, builderTracing_1.recordSpanEvent)('channel_start', { channel: 'email' });
                        ...
                        (0, builderTracing_1.recordSpanEvent)('channel_complete', { channel: 'email', sent: emailResult.sent });
                    }
                    catch (error) {
                        const builderError = BuilderError_1.BuilderError.from(error, 'NotificationBuilder', 'sendEmail', {
                            recipients: users.length,
                            template: resolvedContent.email.template,
                        });
                        builderError.log();
                        (0, builderTracing_1.recordSpanEvent)('channel_error', { channel: 'email', error: builderError.message });
                        if (this.options.throwOnError) {
                            throw builderError;
                        }
                ...
                if (this.channels.has('database')) {
                    try {
                        (0, builderTracing_1.recordSpanEvent)('channel_start', { channel: 'database' });
                        ...
                        (0, builderTracing_1.recordSpanEvent)('channel_complete', { channel: 'database', sent: dbResult.sent });
                    }
                    catch (error) {
                        const builderError = BuilderError_1.BuilderError.from(error, 'NotificationBuilder', 'saveToDatabase', {
                            recipients: users.length,
                        });
                        builderError.log();
                        (0, builderTracing_1.recordSpanEvent)('channel_error', { channel: 'database', error: builderError.message });
                        if (this.options.throwOnError) {
                            throw builderError;
                        }
                ...
                (0, builderTracing_1.addSpanAttributes)({
                    ...
                    'notification.sent.total': result.sent.push + result.sent.socket + result.sent.email + result.sent.database,
                    'notification.failed.total': totalFailed,
                });
                return result;
            }));
        });
