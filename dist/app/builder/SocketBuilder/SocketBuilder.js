...
const ThrottleManager_1 = require("./ThrottleManager");
// ==================== SOCKET INSTANCE ====================
let ioInstance = null;
// ==================== METRICS ====================
const metrics = {
...
class SocketBuilderInstance {
     ...
     * @returns SocketBuilder instance for chaining
     */
    emitBatch(event, payloads) {
        const instance = this;
        instance.event = event;
        instance.batchPayloads = payloads;
        return instance;
    }
    // ==================== THROTTLING ====================
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (this.throttleTTL) {
                    ...
                    if (ThrottleManager_1.ThrottleManager.checkAndSet(key, this.throttleTTL)) {
                ...
                if (this.permissionChecker) {
                    const allowed = yield this.permissionChecker();
                    if (!allowed) {
                        result.permissionDenied = true;
                        this.log('debug', 'Permission denied for: ' + this.event);
                        metrics.deniedEmits++;
                        if (this.deniedHandler) {
                            yield this.deniedHandler(this.socket || undefined);
                        }
                        return result;
                    }
                }
                ...
                if (this.batchPayloads.length > 0) {
                    ...
                    for (const payload of this.batchPayloads) {
                        yield this.executeEmit(io, payload);
                    }
                }
                else if (this.payload) {
                    // Single emission
                    yield this.executeEmit(io, this.payload);
                }
                result.success = true;
                metrics.totalEmits++;
    ...
    getThrottleKey() {
        ...
        const parts = [];
        if (this.event)
            parts.push(String(this.event));
        if ((_a = this.room) === null || _a === void 0 ? void 0 : _a.id)
            parts.push(this.room.id);
        return ThrottleManager_1.ThrottleManager.generateKey(parts[0] || 'unknown', ...parts.slice(1));
    }
    executeEmit(io, payload) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const roomName = this.getRoomName();
            if (((_a = this.room) === null || _a === void 0 ? void 0 : _a.type) === 'broadcast') {
                // Broadcast to all
    ...
    updateEventMetrics() {
        ...
        if (room) {
            const roomType = ((_a = this.room) === null || _a === void 0 ? void 0 : _a.type) || 'unknown';
    ...
    log(level, message) {
        ...
        switch (level) {
            case 'debug':
                logger_1.logger.debug(prefix + ' ' + message);
                break;
            case 'info':
                logger_1.logger.info(prefix + ' ' + message);
                break;
            case 'error':
                logger_1.logger.error(prefix + ' ' + message);
                break;
        }
    }
}
...
exports.SocketBuilder = {
    ...
    initialize(io) {
        ioInstance = io;
        logger_1.logger.info('[SocketBuilder] Initialized with Socket.IO instance');
    },
    /**
     * Get current metrics
     */
     ...
     */
    getThrottleKey(event, ...identifiers) {
        return ThrottleManager_1.ThrottleManager.generateKey(event, ...identifiers);
    },
    /**
...
exports.default = exports.SocketBuilder;
